...
<style>
.main-container { width: 1600px; max-width:3200px;}
</style>  
...
The FASTQ files for this study are available from the NCBI Sequence Read Archive
ID: [PRJNA661407](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA661407)

The associated github repository can be found [here](https://github.com/clarke-lab/CHO_cell_scRNASeq/)

# Prepare for Analysis

## Load packages
```{r load_packages, message=FALSE, warning=FALSE, include=FALSE, echo=TRUE}
package_list <- c("monocle", "plyr", "dplyr", "ggExtra", "reshape", "colorRamps", "stringr", "BioParallel", "ggpubr", "DT", "WebGestaltR", "ggalt", "biomaRt", "patchwork", "viridis", "gridExtra", "grid", "gganimate", "hrbthemes", "jcolors", "cowplot", "data.table", "GGally", "ggplotify", "writexl", "Seurat")
...
color.gradient <- scale_color_gradientn(
...
The primary analysis package for this analysis [Monocle v2](http://cole-trapnell-lab.github.io/monocle-release/docs/) . To use Monocle we must first create a *cell dataset* (CDS) object containing the UMI data. During this process we assign a detection limit along with the expressionFamily - in this case *negbinomial()*.  
```{r create_CDS}
...
```{r scrnaseq_bulk_cor, fig.height=6, fig.width=6}
...
```{r total_umi_filter, fig.height=4, fig.width=4}
...
```{r total_mito_filter, fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
...
```{r filter_cells, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
...
```{r filter_genes, fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
...
### Figure 2
```{r figure_2, fig.height=6, fig.width=10}
p1 <- (ggplot() +
  theme_void() +
  labs(tag = "(a)")) / (ggplot() +
  theme_void())
...
```{r create_seurat_object, message=FALSE, warning=FALSE}
...
```{r normalize_data_in_seurat, message=FALSE, warning=FALSE, include=FALSE}
...
```{r convert_cc_genes_to_CHO, message=FALSE, warning=FALSE}
...
```{r calculate_cc_scores, fig.height=6, fig.width=12, message=FALSE, warning=FALSE}
...
```{r select_variable_genes, fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
...
```{r pca_select_plot, fig.align="center", fig.width=4, message=TRUE}
...
```{r tsne, fig.height=6, fig.width=6, message=FALSE, warning=FALSE}
...
```{r tsne_density_plot, fig.height=5, fig.width=5}
...
```{r cluster_identification, fig.height=5, fig.width=5}
...
```{r figure_3_save, fig.height=9, fig.width=9}
...
Using the genes found to vary beteen the identified clusters we first project onto a 2D space using the DDRTree algorithm before ording the cells in
pseduotime
```{r generate_monocle_trajectory, warning=FALSE}
de_genes <- cluster_de %>%
  arrange(qval) %>%
  filter(qval < 0.01)
...
## HC trajectory
```{r hc_trajectory, message=FALSE, warning=FALSE}
...
```{r figure_5_save, fig.height=12, fig.width=10}
...
```{r monocle_pseudotime_de, message=TRUE, warning=FALSE}
