...
***
# Introduction
Chinese hamster ovary cells are the dominant expression host for the production
of recombinant therapeutic proteins.  

This R notebook enables the analysis following analysis enables the reproduction
of the data analysis described in:  


**Understanding the evolution of a clonally-derived mAb-antibody producing CHO cell line using single cell transcriptomics**

In this study, the four biological replicates of the CHOK1 DP12 cell line were
sampled at 72hrs post-seeding and analysed on the BD Rhapsody system.  The FASTQ
data were converted to a cell gene count matrix using the Seven Bridges Gnees BD
Rhapsody WTA pipeline and combined into a single sample. 

```{r}
#grid::grid.raster( tiff::readTIFF( "./figure/myfigure.tif") )
```


![experimental design](images/experimental_design.tiff)  

## Data Availibility

FASTQ files: GSEXXXXX 

# Preparation 
Check for the required packages to run the analysis
...
color.gradient <- scale_color_gradientn(colors = matlab.like(200), trans = "log10")
...
Create a function to determine the relative expression of a given gene. When the expression of a gene is than the detection limit, the gene is equal to the detection limit.
```{r}
...
# Import raw data
## scRNASeq 
We import data from the [Seven Bridges Genomics](https://www.sevenbridges.com/) (SBG) plaform for analysis in R. The UMI data is stored in a CSV file with each row representing an individual cell, the column represent a gene. In this experiment 4 biological replicates for the CHOK1 DP12 cell line were analysed using Rhapsody WTA. The row label for each cell contains the replicate ID and that ID is used to create a batch ID than be used to regress any technical variation due to batch  
...
The primary analysis package for this analysis monocle version 2. To use this package we must first create a *cell dataset* (CDS) object containing the raw Rhapsody data. During this process we assign a detection limit along an expressionFamily - in this case *negbinomial* as appropriate for UMI data. Negbinomial is more accurate but takes more time than Negbionomial.size()  
```{r create_CDS}
...
Comparison of scRNASeq sample to it’s respective bulkRNASeq sample for each of the CHO K1 D12 cultures analysed.
...
fig.mtDNA.frac <- pData(cells) %>%
  ...
  theme_ms() +
  theme(aspect.ratio = 1, legend.position = "none", axis.text = element_text(size = rel(1)), axis.title = element_text(face = "bold", size = rel(1.5))) +
...
fig.pc.variance <- plot_pc_variance_explained(cells.filter, return_all = FALSE) +
  ...
  theme_ms() +
  theme(aspect.ratio = 1, axis.text = element_text(size = rel(1)), axis.title = element_text(face = "bold", size = rel(1.5)))
...
fig.tsne.density <- ggplot(df) +
  geom_point(aes(x, y, col = d), size = 0.8) +
  scale_color_identity() +
  theme_ms() +
  theme(
...
fig.tsne.cluster <- plot_cell_clusters(cells.filter, color = "Cluster", cell_size = 0.8) +
  scale_y_continuous(limits = c(-35, 35)) +
  scale_x_continuous(limits = c(-35, 35)) +
  labs(x = "t-SNE 1", y = "t-SNE 2") +
  guides(color = guide_legend(override.aes = list(size = 5))) +
...
## HC tSNE
```{r}
fig.tsne.rel.hc <- plot_cell_clusters(cells.filter, color = "rel.exprs.hc", cell_size = 0.8) +
  color.gradient +
  theme_ms() +
  theme(
    panel.grid.major = element_blank(),
...
fig.3f <- fig.box.hc.cluster
saveManuscriptPlot(fig.3f, 6, 3)
fig.3f
```

## LC tSNE
```{r}
fig.tsne.rel.lc <- plot_cell_clusters(cells.filter, color = "rel.exprs.lc", cell_size = 0.8) + color.gradient + theme_ms() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    legend.position = "right",
...
fig5.a <- as.ggplot(fig.trajectory.state)
...
fig4.b <- fig.bar.state.cells
saveManuscriptPlot(fig4.b, 6, 6)
fig4.b
```
...
Those genes that significantly vary with Pseudotime are identified 
...
# write the supplementary table S4
...
```{r}
# relative expression
pData(cells.filter)$rel.exprs.tomm20 <- monocle_relative_expression(cells.filter, "Tomm20")

fig.traj.rel.tomm20 <- plot_cell_trajectory(cells.filter, 
                                              theta = 0, 
                                              show_branch_points = FALSE, 
                                              color_by = "rel.exprs.tomm20", 
                                              cell_size = 0.8) +
                                              color.gradient + 
                                              labs(x = "Component 1", 
                                                   y = "Component 2",
...
fig6.d <- fig.traj.rel.tomm20
saveManuscriptPlot(fig6.d, 7, 3)
fig6.d
```
### Hmox1
```{r}
